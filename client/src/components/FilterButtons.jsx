...
const FilterButtons = ({filterData,flag}) =>{
    ...
    return (
        <div className="border border-gray-300 rounded-md px-4 py-1 relative cursor-pointer 
    ...
    {filterName &&(
      ...
      <>
          {filterName.length > 12 ?  `${filterName.slice(0, 11)}...`
          :filterName
        }
      </>
    ) }
      ...
      {filterData && filterMenu && (
        ...
        >
          {filterData?.map((data) => (
            <div 
              key={data.name}
              className="flex items-center gap-2 px-4 py-1 hover:bg-gray-200"   onClick={() => updateFilterButton(data.name)} >
